...
import csv
import logging
from cognee.modules.search.types import SearchType  # Para usar os tipos de busca
...
async def main():
    ...
    csv_output_filename = 'respostas_v8.csv'
    ontology_file = 'schemaorg.owl'  # Arquivo da ontologia

    # --- CARREGA ONTOLOGIA NO GRAFO ---
    try:
        with open(ontology_file, 'r', encoding='utf-8') as f:
            ontology_content = f.read()
        await cognee.add(ontology_content)  # Remove o 'metadata'
        await cognee.cognify()
        print(f"Ontologia '{ontology_file}' carregada com sucesso no grafo!")
    except FileNotFoundError:
        print(f"ERRO: Arquivo de ontologia '{ontology_file}' não encontrado.")
        return
    ...
    try:
        ...
        print(f"ERRO: O arquivo '{json_input_filename}' não foi encontrado.")
        return
    except json.JSONDecodeError:
        print(f"ERRO: O arquivo '{json_input_filename}' contém um JSON inválido.")
        return
    ...
    for i, record in enumerate(records):
        ...
        try:
            ...
            print(f"AVISO: Registro {i + 1}/{total_records} pulado. Chave ausente: {e}")
            continue

        print(f"Processando registro {i + 1}/{total_records}: {query_text}")

        text_parts = [f"{title}\n{' '.join(sentences)}" for title, sentences in context_data]
        text = "\n\n".join(text_parts)

        # --- Adiciona contexto do registro ao grafo ---
